...
import { addLocalCategory, addLocalProject, removeExpenseRecord, setExpenseData } from '../../../store/slices/expenseSlice';
...
export default function ExpensesContainer() {
  ...
  const handleDelete = async (id: number) => {
    ...
    Swal.fire({
      title: isArchivedView ? 'Delete Permanently?' : 'Archive Record?',
      text: isArchivedView
        ? 'This record will be permanently removed from Archived expenses.'
        : 'This record will be moved to the Archived tab.',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#ef4444',
      cancelButtonColor: '#94a3b8',
      confirmButtonText: isArchivedView ? 'Yes, Delete it!' : 'Yes, Archive it!',
      ...
      try {
        ...
        if (isArchivedView) {
          dispatch(removeExpenseRecord(id));
        }

        Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 }).fire({
          icon: 'success',
          title: isArchivedView ? 'Deleted permanently' : 'Moved to Archived'
        });
      } catch (error: any) {
  ...
  const handleRestore = async (id: number) => {
    ...
    Swal.fire({
      ...
      try {
        await axios.post(`expenses/${id}/restore`);
        Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 }).fire({
          icon: 'success',
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex items-center gap-8 px-2 min-w-max">
          {[
            ...
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as 'active' | 'archived')}
                className={cn(
      ...
      <ExpensesTable
        ...
        onView={(expense) => { setSelectedExpense(expense); setIsViewOpen(true); }}
        onEdit={canManage ? (expense) => { setSelectedExpense(expense); setIsEditOpen(true); } : undefined}
