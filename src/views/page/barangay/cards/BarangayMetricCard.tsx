...
export const BarangayMetricCard: React.FC<MetricCardProps> = ({ icon, title, value, color, bgColor, isLoading }) => {
  return (
    <div className="p-6 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[1.5rem] flex items-center gap-4 shadow-sm relative overflow-hidden group">
      ...
      <div className={cn(
        `p-4 rounded-2xl ${bgColor} ${color} transition-all duration-500`, 
        isLoading && "animate-pulse opacity-60"
      )}>
        {icon}
      </div>
      
      <div className={cn("flex-1 w-full transition-all duration-300", isLoading && "opacity-60")}>
        {isLoading ? (
