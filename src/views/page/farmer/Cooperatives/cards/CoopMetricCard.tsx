...
const CoopMetricCard: React.FC<MetricCardProps> = ({ icon, title, value, color, bgColor, isLoading }) => (
  <div className="p-6 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[1.5rem] flex items-center gap-4 shadow-sm relative overflow-hidden group">
    ...
    <div className={cn("transition-all duration-300", isLoading && "opacity-60")}>
      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{title}</p>
      <h3 className="text-2xl font-black text-gray-800 dark:text-white leading-none">
        {isLoading ? <div className="h-6 w-12 bg-gray-200 dark:bg-slate-800 rounded animate-pulse"/> : value}
      </h3>
    </div>
  </div>
);

export default CoopMetricCard;